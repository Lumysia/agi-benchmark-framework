...
MAINTENANCE_CRITERIA = ["code_usability_checked", "feedback_channel", "contact_person"]
...
class UGFValidator:
    ...
    def _validate_score(self, score: Any, filename: str, criterion: str) -> bool:
        ...
        self._add_error(
            filename,
            f"Invalid score value '{score}' for criterion '{criterion}'. Must be null, 'n/a', or one of {sorted(VALID_SCORES)}",
        )
        return False
    ...
    def _validate_benchmark_info(self, data: Dict[str, Any], filename: str) -> bool:
        ...
        for field in BENCHMARK_INFO_REQUIRED:
            if field not in benchmark_info:
                self._add_error(
                    filename, f"Missing required field '{field}' in 'benchmark_info'"
                )
                valid = False
            elif not benchmark_info[field]:
                self._add_error(
                    filename, f"Field '{field}' in 'benchmark_info' cannot be empty"
                )
                valid = False

        return valid

    def _validate_section_criteria(
        self, data: Dict[str, Any], filename: str, section: str
    ) -> bool:
        ...
        for criterion in expected_criteria:
            if criterion not in section_data:
                self._add_error(
                    filename, f"Missing criterion '{criterion}' in '{section}' section"
                )
                valid = False
                continue
            ...
            if not isinstance(criterion_data, dict):
                self._add_error(
                    filename,
                    f"Criterion '{criterion}' in '{section}' must be a dictionary",
                )
                valid = False
                continue
            ...
            if "score" not in criterion_data:
                self._add_error(
                    filename,
                    f"Missing 'score' field for criterion '{criterion}' in '{section}'",
                )
                valid = False
            else:
                # Validate score value
                if not self._validate_score(
                    criterion_data["score"], filename, f"{section}.{criterion}"
                ):
                    valid = False
            ...
            if "justification" not in criterion_data:
                self._add_error(
                    filename,
                    f"Missing 'justification' field for criterion '{criterion}' in '{section}'",
                )
                valid = False
            elif not criterion_data["justification"]:
                self._add_error(
                    filename,
                    f"Field 'justification' for criterion '{criterion}' in '{section}' cannot be empty",
                )
                valid = False
