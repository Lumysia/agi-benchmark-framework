...
def extract_scores(data: Dict[str, Any], section: str) -> Dict[str, Optional[int]]:
    """
    Extract scores from a section (design, implementation, documentation, maintenance).
    ...
    if section in data:
        for criterion, value in data[section].items():
            if isinstance(value, dict) and 'score' in value:
                ...
                if score is None or (isinstance(score, str) and score.lower() in ['null', 'n/a', 'na']):
...
def calculate_usability_score(sI: float, sDo: float, sM: float, 
                             nI: int, nDo: int, nM: int) -> float:
    """
...
def main():
    ...
    output_file = project_root / 'results' / 'benchmark_evaluation_engineering_scores.csv'
