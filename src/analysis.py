...
def flatten_benchmark_data(benchmarks: List[Dict[str, Any]]) -> pd.DataFrame:
    ...
    for item in benchmarks:
        record = {
            ...
            "eng_reproducibility": item.get("evaluation_engineering", {})
            .get("implementation", {})
            .get("reproducibility"),
            "eng_contamination": item.get("evaluation_engineering", {})
            .get("maintenance", {})
            .get("contamination_status"),
            "eng_status": item.get("evaluation_engineering", {})
            .get("maintenance", {})
            .get("status"),
            "assess_static": item.get("capability_assessment", {})
            .get("static_knowledge", {})
            .get("evaluates"),
            "assess_process": item.get("capability_assessment", {})
            .get("process_orientation", {})
            .get("evaluates"),
            "assess_learning": item.get("capability_assessment", {})
            .get("learning_adaptability", {})
            .get("evaluates"),
            "learning_metrics": [
                metric.get("id")
                for metric in item.get("capability_assessment", {})
                .get("learning_adaptability", {})
                .get("metrics", [])
            ],
            "workflow_stages": len(
                item.get("capability_assessment", {})
                .get("process_orientation", {})
                .get("workflow_stages", [])
            ),
        }
...
def run_analysis(df: pd.DataFrame):
    ...
    if learning_df.empty:
        print(
            "SYSTEMIC GAP: No benchmarks found that evaluate 'Learning Adaptability'."
        )
    else:
        metrics_summary = {
    ...
    if process_df.empty:
        ...
        process_summary = df.pivot_table(
            index="name", values="workflow_stages", aggfunc="sum"
