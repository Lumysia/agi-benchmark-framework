...
def test_extract_scores(mock_benchmark_sections):
    ...
    missing_section_scores = extract_scores(mock_benchmark_sections, 'non_existent_section')
...
def test_process_benchmark(mock_load_yaml, mock_valid_benchmark_data):
    ...
    assert result['sD'] == (10+5+15+0+10+5+15+10+5+15+10+0+10) / 13
    
    assert result['nI'] == 11
    assert result['sI'] == (15+10+5+0+15+10+5+15+10+5+0) / 11
    
    assert result['nDo'] == 16
    assert result['sDo'] == (5+10+0+15+10+5+15+10+15+15+0+5+10+10+5+5) / 16
